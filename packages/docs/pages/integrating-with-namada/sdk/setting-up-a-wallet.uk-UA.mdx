...
### Імпорт для конкретних гаманців
...
## Створення гаманця з мнемоніки

SDK може створити гаманець з мнемонічної фрази. Мнемонічна фраза - це фраза з 24 слів, яку можна використовувати для відновлення гаманця. 
...
У другій частині вищевказаної функції ключ отримується з мнемонічної фрази. Псевдонім - це ім'я ключа, який буде зберігатися в гаманці. Шлях_походження - це шлях до ключа у гаманці HD. Мнемонічна фраза - це мнемонічна фраза, яка була згенерована раніше. Shielded_ctx - контекст для екранованих транзакцій. NullIo - контекст вводу-виводу для гаманця.

## Створення нового гаманця і збереження його у файловій системі

Також можна створити гаманець sdk з нуля. Це більш складний процес, оскільки вимагає створення нового сховища для існування гаманця.
...
Наведений вище код дозволяє нам створити будь-який екземпляр `SdkWallet`, просто передавши секретний ключ і адресу для токена `NAM`. Якщо ми захочемо здійснювати перекази за допомогою інших токенів, нам потрібно буде додати і ці адреси.
