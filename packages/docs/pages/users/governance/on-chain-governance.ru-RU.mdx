...
Все различные типы предложений перечислены в [спецификации](https://specs.namada.net/governance/proposal). Различные предложения имеют разные права доступа, структуры данных, а также требования к голосованию.
...
<Callout type="info">
**Примечание**: 
Кодировка будет представлена в виде строки.
</Callout>
...
где `--vote` может быть либо "да"shell copy Проверка результата
...
```shell copy
unamada client query-proposal-result --proposal-id 0
```