...
Нижче наведено приклад `PGFProposal`(фінансування суспільних благ пропзиція ), який може подати стюард. Зверніть увагу, що тільки стюарди зможуть подавати ці пропозиції.
```json
{
    "proposal" :{
        "content": {
...
```
де `<адреса одержувача>` має бути змінено на адресу одержувача коштів.
