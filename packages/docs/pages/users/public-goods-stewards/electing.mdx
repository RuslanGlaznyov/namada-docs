...
# Як стати управителем

Опікун суспільних благ може складатися з довільної кількості людей, а може бути й однією особою. Єдина вимога полягає в тому, що мультипідписний акаунт стюарда має бути обраний громадою через пропозицію щодо управління.
...
облікового запису мультипідпису зі списку стюардів. У цьому випадку `"дія"` - це `"додати"`, 
а "адреса" - це адреса облікового запису мультипідпису, який буде обрано стюардом. 
Якщо "дія"` буде "видалити"`, то "адреса"` буде адресою облікового запису мультипідпису 
...
```
де `<шлях/до/steward_proposal.json>` - шлях до файлу `steward_proposal.json`.

### Як стати обраним

Після того, як пропозицію буде подано, за неї проголосує спільнота (див. [голосування](./voting.mdx)). Якщо пропозиція пройде, обліковий запис стане стюардом. Якщо пропозиція не пройде, обліковий запис не стане стюардом.
...
Існує 4 способи, як стюард може втратити своє повноваження:
