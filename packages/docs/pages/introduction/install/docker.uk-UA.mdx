...
```bash copy
docker run -P -i -t $DOCKER_IMAGE <команда namada>
```

Де `<команда namada>` - це будь-яка команда, яку ви запустите після `namada` у терміналі. Наприклад, якщо ви хочете виконати `namada client utils join-network --chain-id $CHAIN_ID`, вам слід виконати `namada client utils join-network`:
